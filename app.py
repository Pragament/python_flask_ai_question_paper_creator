...
def select():
    ...
    try:
        response = requests.get(PAGE_ATTRIBUTES_API)
        response.raise_for_status()
...
def generate():
    ...
    system_prompt = (
        ...
        "  ]\n"
        "}\n"
        "Now generate questions based on this input:"
    )
...
def download_pdf():
    ...
    try:
        # If paper.json exists and contains questions, generate the PDF
        ...
        if not os.path.exists(pdf_path):  # Check if the PDF exists
            generate_pdf(paper_json, pdf_path)
